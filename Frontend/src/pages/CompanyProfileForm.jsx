...
function CompanyProfileForm() {
  ...
  const handleSave = (e) => {
    e.preventDefault();
    console.log('Saving data:', coData);
  ...
  return (
    <div className="bg-lgray">
      <div className="py-[106px] px-[50px] sm:px-[85px]">
        ...
        <form className="flex flex-col sm:gap-[123px] sm:flex-row sm:relative" method="POST" action="/company">
          {/* Left Side */}
          ...
          <div className="w-full sm:w-1/2 sm:min-w-0">
            ...
            <div className="flex justify-end pt-[153px] gap-2">
              <button onClick={handleSave} className="custom-btn btn-dpink btn-dpink:hover">Save</button>
              <button onClick={handlePublish} className="custom-btn btn-black btn-black:hover">Publish</button>
            </div>
            ...
            {showConfirmPublishPopup && (
              <>
                <div className="fixed inset-0 backdrop-blur-sm bg-black/30 z-40"></div>
                <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
                  bg-white text-black px-6 sm:px-10 py-3 sm:py-5 rounded shadow-lg z-50
